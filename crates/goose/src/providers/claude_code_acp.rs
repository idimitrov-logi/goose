...
use crate::acp::{AcpProvider, AcpProviderConfig, PermissionMapping};
use crate::config::search_path::SearchPaths;
use crate::config::{Config, GooseMode};
use crate::model::ModelConfig;
use crate::providers::base::{ProviderFactory, ProviderMetadata};
...
impl ProviderFactory for ClaudeCodeAcpProvider {
    ...
    fn from_env(model: ModelConfig) -> BoxFuture<'static, Result<AcpProvider>> {
        Box::pin(async move {
            let config = Config::global();
            let resolved_command = SearchPaths::builder().with_npm().resolve("npx")?;
